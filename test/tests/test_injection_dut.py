"""L1 DUT proofs for injection sync-latency and capture-uncertain START.

Planning unit tests cannot see the two-flop host synchronizer. These cases
prove the Wave 3 driver contracts on ``tb_top``:
...
* raw BUS_REQ is asserted ``SYNC_LATENCY_CYCLES`` before the synchronized
  ...
  remain reachable)
* capture-uncertain START cannot produce two synchronized edges, even when
  the plan carries a large deassert-phase offset
...
Forced ``rst_n=0`` is not used here, so dispose stays at the default
``FORBID`` (no ``RESET-TRUNCATED`` window).
"""
...
from common.directed import auto_timeout_ns, install_chain, wait_for_done_pulse
...
async def injection_start_uncertain_single_edge(dut):
    ...
    # A hold-plus-deassert sum that would span two rising clk edges if the
    # driver added deassert jitter to the short hold.
    ...
    bringup = await bring_up_top(dut)
    chain = build_directed_chain(
        [TcdSpec(transfer_len=1, pattern=PATTERN_INCREMENT)], seed=7201
    )
    install_chain(bringup, chain)

    record = await jitter_start(dut, plan, clk_period_ns=period)
