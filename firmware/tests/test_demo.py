...
from firmware.demo import DEMO_DEST, DEMO_PATTERN, build_demo_memory, main
...
def test_demo_vector_is_psram0_copy_plus_quit():
    ...
    head = mem.read(0, 0, 11)
    assert head[10] & 0x10 == 0  # not QUIT at head
    quit_raw = mem.read(0, 0x0B, 11)
    assert quit_raw[10] & 0x10  # QUIT
    assert mem.read(0, 0x100, len(DEMO_PATTERN)) == DEMO_PATTERN
