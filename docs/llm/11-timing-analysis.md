...
| `T-HZ` | APS6404L `tHZ` is at most 5.5 ns from CE# high to SIO high impedance; physical turnaround must avoid contention with another PSRAM or MCU pass-through | STA for ASIC OE/reclaim plus demoboard | Yes - modeled `tHZ`, ownership resolution, and optional qualified SDF pre-check handoff behavior | Worst-case device release, ASIC ownership delay, and board flight leave a contention-free turnaround interval | todo |
