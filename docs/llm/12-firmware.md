...
8. QPI-write the `MemoryImage`; Hi-Z; drop REQ; wait grant low; `pulse_start`; `wait_idle_after_start` (DONE low then high, or missed low treated as already idle).
...
| `firmware/runner.py` | Grant, install, START, wait idle, dump, compare | Via asic/psram |
...
Unused host bits tied 0 (D34). START capture is the GPIO write duration, not a 1 us hold (`Host.pulse_start` default `hold_us=0`). Shortest chain (head `QUIT`) can finish in about 1.2 us; firmware must not require sampling the DONE-low pulse.
