...
| `ui_in[1]` | Reserved (was ABORT; removed D23) |
...
- QSPI address phase uses **`ptr[22:0]`** (device `A[22:0]`); **`ptr[23]` unused** (drive 0).
...
Frozen: `ui_in[0]=START`, `ui_in[2]=BUS_REQ`, `uo_out[0]=DONE`, `uo_out[1]=BUS_GNT` (D14/D18/D22/D23); `ui_in[1]` reserved; no ABORT pin; no head-pointer pins. Per TinyDMA-2C prior art, that design used `ui_in` + `uio` strobes with a command/payload config adapter and a **fixed** SPI `uio_oe` mask; this project needs dynamic SIO OE for QSPI and a shared-bus pass-through model instead. Same I/O scarcity applies.
...
2. **Address** - 24-bit phase on the wire (6 cycles); useful bits are `A[22:0]`; phase MSB unused (`addr[23]=0`); device via `device_sel`
