...
6. **Firmware note (D30):** shipped MCU code still issues `0x35` (SPI Enter Quad) and `0xF5` (QPI Exit Quad) plus SPI `0x66`/`0x99` reset. The ASIC still emits none of those. MCU install/dump after enter is QPI `0xEB`/`0x02` (same data opcodes as the ASIC).
...
5. Host-side **pytest** of pure firmware logic under `firmware/tests/` is required (no demoboard hardware, no `test/` imports). There is **no** `firmware/cases.py` and **no** one function per directed-test `TC-*` ID: `build.py` assembles arbitrary chains; **one** canned demo is `firmware/demo.py`. Demoboard HIL remains Phase 3 / M7 (D28). Deploy is UF2 + `mpremote` copy of `firmware/`; there is no compile of this project's Python.

**Why:** Locks the MicroPython / ETR / `firmware/` workstream to QPI install/dump after MCU-owned enter/exit (D17), reuses the published PIO catalog, forbids `test/` imports on the MCU tree, avoids a per-ID firmware catalog, avoids unnecessary flash QE programming on store Pmods, and keeps unit-testable logic separate from M7 HIL.
