...
5. **Byte copy vs refresh / pages:** V1 scratch is **1 byte**, so each data phase is cmd+addr(+dummy)+1 byte then CE# high. Device limits only matter if `N` grows: **`N ≥ 60`** can exceed `tCEM` 4 us on `0xEB` @ 33 MHz SCK; **`N ≥ 1026`** can cross two 1K pages. Data beats are QPI-only (`0xEB`/`0x02`). MCU QPI chunking vs `tCEM` is a firmware concern ([`12-firmware.md`](12-firmware.md)); do not treat D20 as the MCU chunk policy.
