...
6. **Pass-through / bus OE (D22/D26):** demoboard shares `uio` among RP2040, ASIC, and PSRAM/flash PMOD. Pass-through is `BUS_REQ`/`BUS_GNT`: MCU drives only while granted. While `~BUS_GNT`, ASIC is the **bus keeper** (park all CS high / SCK low; SIO drives a don't-care except floating for dummy/read during a live txn). Board has **10 kΩ** pull-ups on each CS for reset / pre-enable. Both masters enabled with disagreeing levels is contention - see host-interface / firmware docs.
