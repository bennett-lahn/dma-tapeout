...
6. **Pass-through / bus OE (D22/D26):** demoboard shares `uio` among RP2040, ASIC, and PSRAM/flash PMOD. Pass-through is `BUS_REQ`/`BUS_GNT`: MCU drives only while granted. While `~BUS_GNT`, ASIC is the **bus keeper** (park all CS high / SCK low; SIO don't-care in park after `tHZ`; float for dummy/read and through post-CE# `tHZ`). Board has **10 kΩ** pull-ups on each CS for reset / pre-enable. Both masters enabled with disagreeing levels is contention - see host-interface / firmware docs. Normative ownership matrix: [`03-architecture.md`](03-architecture.md).
