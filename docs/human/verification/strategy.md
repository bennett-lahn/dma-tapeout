...
- L2 entry: `source test/env.sh && test/scripts/run_gl.sh` (or `GATES=yes make` / `make gl_test` from `test/`). Copies the unpowered N=5 `nl` view to `test/gate_level_netlist.v`, requires IHP cell models (`PDK_ROOT`), and runs `tests.test_gate_level`. Bare `make` / Tiny Tapeout `GATES=yes make` uses Makefile `.DEFAULT_GOAL=test` so the official `gl_test` action actually simulates and writes `test/results.xml` (plus `tb.fst` / `tb.vcd` when waves exist). The wrapper does not override netlist `DMA_BUF_DEPTH`; Makefile `-Ptb_gl.DMA_BUF_DEPTH` is not used. Missing netlist or PDK is `blocked`, not a pass.
