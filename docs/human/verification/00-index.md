...
| M5 random / `COV-*` / depth sweep | `pass` | **Exit (2026-08-16):** random green at **N=5** / `ideal` (zero TB placeholders) - Icarus and Verilator seeds 1/2/3/5/8; seed-1 Icarus ≡ Verilator. `TC-DEPTH` (directed suite at each compile-time `DMA_BUF_DEPTH`) **pass** N=1..8 (Icarus, 13/13 per depth via `make depth` / `run_depth_sweep.sh`). `COV-*` (functional coverage point IDs) merge at `test/runs/m5_coverage_closure.json`: `closed=true` (20 catalog IDs hit; 13 exclusions STALL + length-class collapse N=1/2; reviewer `M5-close`, 2026-08-16). A 2026-08-21 rematch of that merge recorded `closed=false`; findings in [`../../review-passthrough-2026-08-21/00-index.md`](../../review-passthrough-2026-08-21/00-index.md) (not a sign-off rewrite) |
| L2 functional infra | `wip` | `tests.test_gate_level` + `test/scripts/run_gl.sh`; flattened N=5 instance (no `#(.DMA_BUF_DEPTH)`); Makefile `.DEFAULT_GOAL=test` so Tiny Tapeout `GATES=yes make` / `make gl_test` actually simulates the L2 subset under `TIMING_PROFILE=ideal` (zero TB placeholders) and copies `test/results.xml` plus `tb.fst`/`tb.vcd`. Zero-delay Icarus `TC-SMOKE` passed 2026-08-17 on the existing unpowered `nl` view (not an SDF pass; full M6 subset and N=5 netlist signoff still open) |
