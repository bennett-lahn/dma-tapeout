...
- `Q-LAUNCH` checks that driven SIO and OE changes obey the low-SCK launch policy and modeled setup/hold windows while the ASIC drives SCK (`asic_sck_oe==1`; not during grant/reset OE clear). L0 has no `asic_sck_oe` because the engine always drives SCK. Zero-event `Q-LAUNCH` is `na`, not pass. Simulation prerequisite is `pass` at M3 (2026-08-10); it must still precede physical `T-SP-HD` closure.
- `Q-RXEDGE` (`Q-RXEDGE`: each launched read nibble captured on the following rising SCK) reconciles each falling-edge PSRAM launch with exactly one following rising-edge DUT capture. L0 uses `rdata_valid` plus nibble match. L1/L2 have no `rdata_valid` alias; the armed external rising SCK is the D16 (66 MHz / rising-edge RX) capture and still applies `tACLK` / `PSRAM_TACLK_NS` (read data valid after falling SCK). Without a timed read stream the check is `blocked`/`na`, not pass. Device-plane CE# commit may re-audit race-window opens under non-zero `D_OUT_*` (`D_OUT_*`: TB output delays toward the device). Simulation prerequisite is `pass` at M3; it must still precede physical `T-ACLK` closure. Parser SCK sampling gates on delayed device-plane CE#, not live DUT-plane CE#, so unequal `D_OUT_CE` / `D_OUT_SCK` (DUT-to-device CE# vs SCK transport delays) cannot drop edge nibbles from source-time CE# gating alone.
...
**Read-path `T-HZ` note:** after a read CE# rise, the engine keeps SIO floated through `SCLK_OFF` / `CS_OFF` and reasserts SIO OE on the next `IDLE` entry - **one `clk`** (~15.2 ns @ 66 MHz) vs APS6404L `tHZ` (chip-disable to DQ high-Z, max 5.5 ns). That is the tightest modeled turnaround; writes keep SIO driven through CE# rise. Phase 3 must account for this margin, not treat it as an RTL defect.
