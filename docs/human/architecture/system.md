...
| `ui_in[0]` | **START** - accepted only while IDLE/`DONE` and `~BUS_REQ`; ignored while busy |
...
- Quit TCD or abort (after current QPI txn) → idle again (DONE)
...
| Block | Job | Detail |
|---|---|---|
| Top / host sync | TT entry; sync async MCU inputs (`START`/`ABORT`/`BUS_REQ`) into `clk` before use | [`blocks/host-interface.md`](blocks/host-interface.md) (sync) |
| Host / mode control | Pass-through vs DMA master, START/DONE/abort (uses synced levels) | [`blocks/host-interface.md`](blocks/host-interface.md) |
...
1. **PSRAM bring-up over SPI** (`BUS_REQ`/`BUS_GNT`): on each die DMA will touch, wait `tPU`, issue Reset Enable (`0x66`) then Reset (`0x99`), then Enter Quad (`0x35`). ASIC expects both dies already in **QPI** before START (D17); this work is MCU-only
2. **Create TCDs** in PSRAM under grant; first TCD (or `QUIT` TCD) at `0x000000` on PSRAM 0; end chain with `QUIT=1`
...
5. Wait for **DONE** again, assert **ABORT** (`ui_in[1]`), or pause mid-run with **BUS_REQ**
...
- Status / DFT packing on `uo_out[7:2]` (and optional `ui_in[7:3]`)
- Self-pointing descriptor policy vs abort

Settled for V1: **24-bit** pointers with **`ptr[23]` device**; **11-byte** TCD with **`QUIT`** flag; fixed head at 0/PSRAM0; zero-length no-op; idle/START/ABORT/DONE (D14/D18/D19); **BUS_REQ/BUS_GNT** pass-through (D22); QPI data `0xEB`/`0x02` (D15/D17); **MCU** enter/exit QPI (D17); **66 MHz `clk`**, **SCK = clk/2**, rising-edge RX (D16); D21 handshake; **1-byte** data buffer (D20); `ui_in[0]=START`, `ui_in[1]=ABORT`, `ui_in[2]=BUS_REQ`, `uo_out[0]=DONE`, `uo_out[1]=BUS_GNT`; QSPI on `uio`; **dual PSRAM** DMA; **ASIC flash unsupported**. Post-V1 ladder: [`post-v1.md`](post-v1.md).
